...
export function renderMarkerPreviewHTML(event: Event): string {
  ...
  return `
    <div style="width:220px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
      ${imgHtml}
      <div style="padding:8px 10px 10px;">
        <h4 style="font-size:13px;font-weight:600;color:#111827;margin:0 0 4px;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">
          ${event.title}
        </h4>
        ${event.venue_name ? `<p style="font-size:11px;color:#6b7280;margin:0 0 3px;">${event.venue_name}</p>` : ''}
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;">
          ${price ? `<span style="font-size:12px;font-weight:600;color:${event.is_free ? '#22c55e' : '#374151'};">${price}</span>` : ''}
          ${event.age_label ? `<span style="font-size:11px;color:#9ca3af;">${event.age_label}</span>` : ''}
        </div>
        <button
          class="marker-preview-details-btn"
          style="width:100%;padding:5px 0;background:#e91e63;color:white;border:none;border-radius:6px;font-size:12px;font-weight:500;cursor:pointer;"
        >
          View details
        </button>
      </div>
